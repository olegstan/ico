			...
			<form id="wp-clanwars-manageform" action="admin-post.php" method="post">
				...
				<?php
					foreach ( $table_columns as $column_key => $column_display_name ) {
						...
						?>
						<th scope="col" <?php echo $class; ?> style="<?php echo esc_attr($style); ?>"><?php echo $column_display_name; ?></th>
					<?php }
				...
				<?php foreach($teams as $i => $item) : ?>
					...
					<tr class="iedit<?php if($i % 2 == 0) echo ' alternate'; ?>">
						...
						<td class="logo column-logo">
							<?php echo isset($item->logo) ? wp_get_attachment_image($item->logo, array(56,56)) : ''; ?>
						</td>
