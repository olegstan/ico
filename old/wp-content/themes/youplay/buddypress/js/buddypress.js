...
jq(document).ready( function() {
	...
	$whats_new.focus( function(){
		...
		if ( $activity_all.length  ) {
			if ( ! $activity_all.hasClass( 'active' ) ) {
				// reset to everything
	...
	jq('#activity-filter-select select').change( function() {
		var selected_tab = jq( 'div.activity-type-tabs li.active' ),
			filter = jq(this).val(),
			scope;
			...
			function(response) {
				...
				if ( 'fav' === type ) {
					...
					if ( !Number( jq('.item-list-tabs ul #activity-favorites span').html() ) ) {
						if ( jq('.item-list-tabs ul #activity-favorites').hasClass('active') ) {
				...
				if ( 'activity-favorites' === jq( '.item-list-tabs li.active').attr('id') ) {
	...
	jq( '.dir-search, .groups-members-search' ).on( 'click', function(event) {
		...
		if ( target.attr('type') === 'submit' ) {
			css_id = jq('.item-list-tabs li.active').attr('id').split( '-' );
			object = css_id[0];
			template = null;
	...
	jq('li.filter select').change( function() {
		...
		if ( jq('.item-list-tabs li.active').length ) {
			el = jq('.item-list-tabs li.active');
		} else {
			el = jq(this);
		}
	...
	jq('#send-invite-form').on( 'click', '#invite-list input', function() {
		...
		if ( ! invites_new_template ) {
			jq( '.item-list-tabs li.active' ).addClass( 'loading' );
		}
		...
		function(response)
		{
			...
			if ( invites_new_template ) {
				...
				jq('.item-list-tabs li.active').removeClass('loading');
			}
		});
	});
...
function bp_init_activity() {
	...
	if ( undefined !== jq.cookie('bp-activity-scope') && jq('.activity-type-tabs').length ) {
		...
		jq('#activity-' + jq.cookie('bp-activity-scope') + ', .item-list-tabs li.current').addClass('active');
	}
}
...
function bp_init_objects(objects) {
	jq(objects).each( function(i) {
		...
		if ( undefined !== jq.cookie('bp-' + objects[i] + '-scope') && jq('div.' + objects[i]).length ) {
			...
			jq('#' + objects[i] + '-' + jq.cookie('bp-' + objects[i] + '-scope') + ', #object-nav li.current').addClass('active');
		}
	});
}
...
function bp_filter_request( object, filter, scope, target, search_terms, page, extras, caller, template ) {
	...
	jq('#' + object + '-' + scope + ', #object-nav li.current').addClass('active');
	jq('.item-list-tabs li.active').addClass('loading');
...
function bp_activity_request(scope, filter) {
	...
	jq('.item-list-tabs li').each( function() {
		jq(this).removeClass('active loading');
	});
	/* Set the correct selected nav and filter */
	jq('#activity-' + scope + ', .item-list-tabs li.current').addClass('active');
	jq('#object-nav.item-list-tabs li.active, div.activity-type-tabs li.active').addClass('loading');
